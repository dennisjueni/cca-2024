...
def plot():
    plot_with_args("A", 1, [0, 1, 2])
    plot_with_args("B", 1, [0, 1, 2])
    plot_with_args("A", 10, [0, 1, 2])
    plot_with_args("B", 10, [0, 1, 2])


def plot_with_args(PART: Union[Literal["A"], Literal["B"]], INTERVAL: int, RUNS: list[int]):
    ...
    for RUN in RUNS:
        ...
        if PART == "A":
            ...
            for run in runs:
                if run[2] == "memcached":
                    mc_xs.append((run[0] - min(start_times)).total_seconds())

                    # run[5] contains a list with the cores
                    mc_ys.append(len(run[5]) * 1000)

            mc_xs.append(MAX_LEN)
            mc_ys.append(2000)

            ax1.step(mc_xs, mc_ys, color=c_dict["num_cores"], where="post")

        ax2.step(xs, qs, color=c_dict["QPS"], alpha=0.75 if INTERVAL == 1 else 1)

        HEIGHT = 100
        HEIGHT_NO_OFFSET = 90
        MARGIN = 100
        NODE_A_START = MARGIN + max_left
        ...
        if PART == "A":
            ...
            yticks = [1000, 2000]
            ylabels = [1, 2]
            ax1.set_yticks(yticks, labels=ylabels, color=c_dict["num_cores"], minor=False)
        ...
        if PART == "A":
            ...
            ax1.set_ylabel("Number of cores", color=c_dict["num_cores"], fontdict={"fontsize": 16})
        ...
        plt.savefig(fname=f"{BASE_DIR}/int{INTERVAL}_run{RUN}/{RUN+1}{PART}.pdf")
